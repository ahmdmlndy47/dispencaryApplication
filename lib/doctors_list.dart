...
class _DoctorsListState extends State<DoctorsList> {
  ...
  getDoctors() async{
    ...
    if(doctorsSnapshot.docs.isEmpty){return;}
                                                  ...
                                                  MyButton(
                                                      onPressed: (){
                                                        ...
                                                        AwesomeDialog(
                                                            ...
                                                            btnOkOnPress: ()async{
                                                              final currDis =await  FirebaseFirestore.instance.collectionGroup("dispensaries").where("admins",arrayContains: FirebaseAuth.instance.currentUser!.uid).limit(1).get();
                                                              final docClinics = await currDis.docs.first.reference.collection("clinics").where("docName",isEqualTo: "${snapshot.data!.docs[index]["firstName"]} ${snapshot.data!.docs[index]["lastName"]}").get();
                                                              //وإزالة العيادات الموجود فيها
                                                              for(final cl in docClinics.docs){
                                                                await cl.reference.delete();
                                                              }
